...
class Map(MapClass):
    ...
    def _circles(self):
        return self.get('circ', [])

    circles = property(_circles)
...
class Circle(MapClass):
    ...
    Equivalent to google.maps.Circle. When parsed by JSONEncoder
    and subsequently by our custom jQuery plugin, it will be
    converted to an actual google.maps.Circle instance.
    ...
    def setOptions(self, options):
        if options and 'map' in options:
            if self._map:
                self._map['circ'].remove(self)
            # Save new map reference.
            self._map = options.pop('map')
            if self._map:
                # Add this marker to the map.
                self._map.setdefault('circ', []).append(self)
