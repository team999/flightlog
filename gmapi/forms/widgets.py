...
class GoogleMap(Widget):
    ...
    def render(self, name, gmap, attrs=None):
        ...
        # hacked around to make the map 100% big --mfincham
        
        style = u'position:relative;width:100%;height:100%;'
        ...
        map_div = u'<div class="%s" style="position:absolute; width:100%%;height:100%%"></div>' % escape(dumps(gmap, separators=(',', ':')))
        map_img = u'Please wait while the map loads...' #u'<img style="position:absolute;z-index:1" width="100%%" height="100%%" alt="Google Map" src="%(map)s&amp;size=%(x)dx%(y)d" />' % {'map': escape(gmap), 'x': width, 'y': height}
